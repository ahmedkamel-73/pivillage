...
export async function onRequestGet({request, env}) {
  ...
  const {results}=await env.DB.prepare("SELECT billboards.*, shops.name as shop_name FROM billboards JOIN shops ON billboards.shop_id=shops.id WHERE billboards.is_active=TRUE AND billboards.starts_at <= ? AND billboards.ends_at > ? ORDER BY billboards.type DESC LIMIT 20").bind(now, now).all();
...
export async function onRequestPost({request, env}) {
  ...
  // Check Pi payment for billboard - in production verify Pi payment
